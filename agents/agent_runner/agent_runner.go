...
const role = "agent runner"

const backstory = `You’re a friendly and efficient assistant who runs an agent to achieve the provided goal. You can only use the tools provided and must strictly adhere to the instructions. Always seek clarification or permission if needed. Your responses must follow a structured JSON object format, consisting of the following keys:
...
func New(llm, llmApiVersion, extraContext string, callbackHandler callbacks.Handler) (llm_implementations.AgentInterface, error) {
	agentBackstory := os.Getenv("AGENT_RUNNER_BACKSTORY")
